    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                #one ant is already in this place.
                assert self.ant.can_contain(insect) or insect.can_contain(self.ant) == True, 'Two ants in {0}'.format(self)
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        ...
        if self.container == True:
            if self.ant is None and other.container == False:
                return True
        return False
        # END Problem 9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0
        place_to_choose = self.place
        while place_to_choose is not hive:
            if self.min_range <= distance <= self.max_range:
                if place_to_choose.bees:
                    return random_or_none(place_to_choose.bees)

            place_to_choose = place_to_choose.entrance
            distance += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            place_to_do = self.place
            for bee in place_to_do.bees[:]:
                bee.reduce_armor(self.damage)
            place_to_do.remove_insect(self)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.index == 1:
            ...
            all_bees_behind = []
            place_to_do = self.place.exit
            while place_to_do.exit is not None:
                temp_ant = place_to_do.ant
                if temp_ant is not None:
                    if isinstance(temp_ant, BodyguardAnt):
                        if temp_ant.ant is not None:
                            all_bees_behind.append(temp_ant.ant)
                            all_bees_behind.append(temp_ant)
                    all_bees_behind.append(temp_ant)
                place_to_do = place_to_do.exit
            for ant in all_bees_behind:
                if ant not in self.double_list:
    ...
    def reduce_armor(self, amount):
        ...
        if self.index == 1 and self.armor <= 0:
            bees_win()
        # END Problem 13
