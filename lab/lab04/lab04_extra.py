...
def merge(lst1, lst2):
    ...
    over_list1, over_list2 = len(lst1), len(lst2)
    result = []
    if over_list1 == 0:
        return lst2
    if over_list2 == 0:
        return lst1
    while index_list1 < over_list1 and index_list2 < over_list2:
        if lst1[index_list1] < lst2[index_list2]:
            result.append(lst1[index_list1])
            index_list1 += 1
        elif lst1[index_list1] == lst2[index_list2]:
            result.append(lst1[index_list1])
            result.append(lst2[index_list2])
            index_list1 += 1
            index_list2 += 1
        else:
            result.append(lst2[index_list2])
            index_list2 += 1
    result = result + lst1[index_list1:] + lst2[index_list2:]
    return result
...
def create_board(rows, columns):
    ...
    i = 0
    while i < rows:
        result.append(create_row(columns))
        i += 1
    return result
...
def put_piece(board, max_rows, column, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    putitem = 'X' if player == 'X' else 'O'
    m = 0
    index = 0  # now in the 0th row of the board.
    flag = False
    # index 实际上多加了一个
    while m < max_rows:
        piece = get_piece(board, index, column)
        if piece == '-':
            flag = True
            index += 1
        m += 1
    ...
    #可以替换
        ...
        #在第一行替换
        if index == 0:
            new_board = replace_elem(board, 0, replace_elem(board[index], column, putitem))
            return 0, new_board
        else:
            new_board = replace_elem(board, index - 1, replace_elem(board[index - 1], column, putitem))
            return index - 1, new_board
