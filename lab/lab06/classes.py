...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        "*** YOUR CODE HERE ***"
        #The first step is to acquire the character object.
    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            if thing in self.place.things.keys():
                take_thing = self.place.things[thing]
                self.backpack.append(take_thing)
    ...
    def unlock(self, place):
        ...
        if place_to_unlock.locked:
            if key is None:
                print(place + " can't be unlocked without a key!")
            else:
                place_to_unlock.locked = False
